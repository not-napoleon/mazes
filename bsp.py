...
class Dungeon(object):
  ...
  def mk_room(self, bounding_box):
    if bounding_box.top + self.edge_min + self.room_min > bounding_box.bottom:
      raise ValueError("Region too small to make room")
    if bounding_box.left + self.edge_min + self.room_min > bounding_box.right:
      raise ValueError("Region too small to make room")
    h_max = bounding_box.bottom - bounding_box.top - self.edge_min
    w_max = bounding_box.right - bounding_box.left - self.edge_min
    h = random.randint(self.room_min, h_max)
    w = random.randint(self.room_min, w_max)
    ...
    v_start = random.randint(bounding_box.top + self.edge_min,
        bounding_box.bottom - h)
    h_start = random.randint(bounding_box.left + self.edge_min,
        bounding_box.right - w)
    room = Box(v_start, h_start, v_start + h - 1, h_start + w - 1)
    for i in range(v_start, v_start + h):
      for j in range(h_start, h_start + w):
        self.set_tile(j, i)
    return room
  ...
  def mk_dungeon(self, bounding_box, depth=0):
    ...
    if ((depth >= self.max_depth)
        or (bounding_box.top + edge_buffer > bounding_box.bottom - edge_buffer)
        or (bounding_box.left + edge_buffer > bounding_box.right - edge_buffer)
        ):
      tries = 0
      while tries < 3:
        try:
    ...
    if split_dir:
      ...
      box_1 = Box(bounding_box.top, bounding_box.left, bounding_box.bottom,
          split)
    ...
    # Now we have two "rooms" (which may be sub-rooms connected by a corridor),
    ...
    if split_dir:
      ...
      if overlap_right >= overlap_left:
        ...
        for i in range(start, end):
          if self.get_tile(corridor_row, i) == '#':
            ...
            # If we started drawing and now hit a non-wall, we're done
            break
      else:
        print "No horizontal overlap between %s and %s" % (room_1, room_2)
    else:
      ...
      if overlap_bottom >= overlap_top:
        ...
        print "picked vertical overlap point %s between %s and %s" % (
            corridor_row, room_1, room_2)
        started = False
        # rooms may be a compound box, in which case room_1.right doesn't
        # border an open tile.  To correct for this, move the start left until
        # we find a non-wall tile.
        start = room_1.right
        ...
        print "No vertical overlap between %s and %s" % (room_1, room_2)
    # print self
    return Box(
